...
{
    return StorageGetById((Storage *)((char *)h + sizeof(IPPair)), STORAGE_IPPAIR, id);
}

int IPPairSetStorageById(IPPair *h, int id, void *ptr)
{
    return StorageSetById((Storage *)((char *)h + sizeof(IPPair)), STORAGE_IPPAIR, id, ptr);
}

void *IPPairAllocStorageById(IPPair *h, int id)
{
    return StorageAllocByIdPrealloc((Storage *)((char *)h + sizeof(IPPair)), STORAGE_IPPAIR, id);
}

void IPPairFreeStorageById(IPPair *h, int id)
{
    StorageFreeById((Storage *)((char *)h + sizeof(IPPair)), STORAGE_IPPAIR, id);
}

void IPPairFreeStorage(IPPair *h)
{
    if (IPPairStorageSize() > 0)
        StorageFreeAll((Storage *)((char *)h + sizeof(IPPair)), STORAGE_IPPAIR);
}
