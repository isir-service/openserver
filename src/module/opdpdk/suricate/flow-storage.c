...
{
    return StorageGetById((Storage *)((char *)f + sizeof(Flow)), STORAGE_FLOW, id);
}

int FlowSetStorageById(Flow *f, int id, void *ptr)
{
    return StorageSetById((Storage *)((char *)f + sizeof(Flow)), STORAGE_FLOW, id, ptr);
}

void *FlowAllocStorageById(Flow *f, int id)
{
    return StorageAllocByIdPrealloc((Storage *)((char *)f + sizeof(Flow)), STORAGE_FLOW, id);
}

void FlowFreeStorageById(Flow *f, int id)
{
    StorageFreeById((Storage *)((char *)f + sizeof(Flow)), STORAGE_FLOW, id);
}

void FlowFreeStorage(Flow *f)
{
    if (FlowStorageSize() > 0)
        StorageFreeAll((Storage *)((char *)f + sizeof(Flow)), STORAGE_FLOW);
}
